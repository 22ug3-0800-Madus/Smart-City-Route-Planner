...
class AVLNode {
    ...
    AVLNode left, right;
    int height;
    AVLNode(Location loc) { this.loc = loc; this.height = 1; }
}
...
public class AVLTree {
    ...
    private AVLNode balance(AVLNode node) {
        ...
        if (bal > 1) { node.left = rotateLeft(node.left); return rotateRight(node); }
        if (bal < -1 && getBalance(node.right) <= 0) return rotateLeft(node);
        if (bal < -1) { node.right = rotateRight(node.right); return rotateLeft(node); }
        return node;
    }
